...
@router.post("/{key_id}", response_class=HTMLResponse)
def update_key(
    request: Request,
    key_id: int,
    session: SessionDep,
    name: str = Form(),
    default_provider: str | None = Form(default=None),
    default_model: str | None = Form(default=None),
    allowed_models: str | None = Form(default=None),
) -> Response:
    user = get_current_user(session, access_token=request.cookies.get("access_token"))
    allowed = [
        item.strip()
        for item in (allowed_models or "").split(",")
        if item.strip()
    ] or None
    error = None
    try:
    ...
    keys = list_user_api_keys(session, user)
    return templates.TemplateResponse(request, "partials/keys.html", {"keys": keys, "error": error})
